...
import PrecipitationData from "../data/precipitation.json"


class PrecipitationGraph extends Component {
  ...
  HoverInfo = ({active, payload, label}) => {
    if (active) {
      ...
      <div className="tooltip">
          ...
          <p>{(payload[0].value.toFixed(2))} mm<sup>2</sup> </p>
      </div>
      );
    }
    return null;
  ...
  render() {
    ...
    let precipitationData = JSON.stringify(PrecipitationData, null, 1)
    let splitData = precipitationData.split(/\r?\n/).slice(13, 4183)
